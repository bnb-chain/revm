...
pub(crate) const DOUBLE_SIGN_EVIDENCE_VALIDATION: PrecompileWithAddress = PrecompileWithAddress(
    crate::u64_to_address(104),
    Precompile::Standard(double_sign_evidence_validation_run),
);
...
fn double_sign_evidence_validation_run(input: &Bytes, gas_limit: u64) -> PrecompileResult {
    ...
    let evidence = match DoubleSignEvidence::decode(&mut input.iter().as_ref()) {
        Ok(e) => e,
        Err(_) => return Err(Error::Reverted(DOUBLE_SIGN_EVIDENCE_VALIDATION_BASE).into()),
    };
    ...
    if header1.number.to_be_bytes().len() > 32 || header2.number.to_be_bytes().len() > 32 {
        return Err(Error::other("invalid evidence").into());
    }
...
mod tests {
    ...
    fn test_double_sign_evidence_validation_run_invalid_evidence() {
        ...
        assert_eq!(res.err(), Some(Error::Reverted(10000).into()));
    }
}
